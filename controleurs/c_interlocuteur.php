<?php 
    require_once("modeles/m_interlocuteur.php");
    $_SESSION['action']();
    ...
    function gestion() {
        $lesInterlocuteurs = getLesInterlocuteurs();
    ...
    function delete() {
        if (isConnect() && isAdmin()) {
            if ($_SERVER["REQUEST_METHOD"] != "POST" && $_POST['idInterDelete']) { 
                $unInterlocuteur = getUnInterlocuteurById($_POST['']);
            }
        }
    }
?>