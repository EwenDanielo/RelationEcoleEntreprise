<div class="table100 ver5 m-b-110" style="margin : 0 15px 0 15px;">
    ...
    <div class="table100-body js-pscroll ps ps--active-y">
        <table>
            <tbody>
                <?php 
                while ($uneAlternance = $lesAlternances->fetch()) {
                ?>
                    <tr class="row100">
                        <td class="cell100" width="15%" align="center"><?php echo $uneAlternance->idA ?></td>
                        <td class="cell100" width="20%"><?php echo $uneAlternance->intituleA ?></td>
                        <?php $leSecteur = getUnSecteurById($uneAlternance->idSecteur); ?>
                        <td class="cell100" width="15%"><?php echo $leSecteur->nomSecteur ?></td>
                        <?php $lEntreprise = getUneEntreprise($uneAlternance->idEnt) ?>
                        <td class="cell100" width="30%"><?php echo $lEntreprise->nomEnt ?></td>
                        <td class="cell100" width="15%">
                            <div class="d-flex flex-row">
                                <div class="p-1">
                                    <form action="<?php echo WEBROOT.'alternance/update' ?>" method="POST">
                                        <input type="hidden" id="idDUpdate" name="idDUpdate" value="<?php echo $uneAlternance->idA ?>">
                                        <span><button type="submit" class="btn btn-primary"><i class="bi bi-pencil-square"></i></button></span>
                                    </form>
                                </div>
                                <div class="p-1">
                                    <form action="<?php echo WEBROOT.'alternance/delete' ?>" method="POST">
                                        <input type="hidden" name="idDelete" value="<?php echo $uneAlternance->idA ?>">
